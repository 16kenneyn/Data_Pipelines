...
logger = DataPipelineLogger("Realtor_Monthly_Hotness_County")
...
def fill_na(dataframe):
    ...
    dataframe["county_name"] = dataframe["county_name"].fillna("")
    dataframe["cbsa_title"] = dataframe["cbsa_title"].fillna("")
...
def chg_data_type(dataframe):
    ...
    temp_df['county_fips'] = temp_df['county_fips'].astype(int)
    temp_df['county_name'] = temp_df['county_name'].astype(str)
    temp_df['cbsa_code'] = temp_df['cbsa_code'].astype(int)
    temp_df['cbsa_title'] = temp_df['cbsa_title'].astype(str)
...
def main_run():
    ...
    sql_table_name = 'REALTOR_MNTHLY_HOT_COUNTY'
    ...
    if sql_table_max_date: # Determining if max date is not None, if it is, the else statement will kick off the history pull.
        redfin_df = get_url(county_history_url)
        ...
        redfin_df = get_url(county_history_url) # Use history load url since no data was returned from sql tale.
...
if __name__ == '__main__':
    ...
    # redfin_df_v1 = get_url(county_history_url)
    #
    # redfin_df_v2 = dataframe_etl(redfin_df_v1)
    # refin_df_v2_0 = fill_na(redfin_df_v2)
    # redfin_df_v3 = chg_data_type(refin_df_v2_0)
    #
    # # Adding the date time stamp
    #
    # redfin_df_v3.to_excel('C:/Users/nickk/PycharmProjects/Data_Pipelines/2_Test/Data_Files/Realtor_Hotness/county_hotness_current_test.xlsx', index=False)
