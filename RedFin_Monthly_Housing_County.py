...
logger = DataPipelineLogger("Redfin_Monthly_Housing_County")
con = duckdb.connect()  # Create a DuckDB connection (in-memory)
universal_url_string = county_url
sql_table_name = 'REDFIN_MNTHLY_HOUSING_COUNTY'  # SQL table name
...
def rdm_transformations(df: pd.DataFrame):
    ...
    SELECT 
        ...
        COALESCE(TRIM(CAST(region_type AS VARCHAR)), 'NA') AS region_type,
        COALESCE(region_type_id, 0) AS region_type_id,
        COALESCE(table_id, 0) AS table_id,
        COALESCE(TRIM(CAST(is_seasonally_adjusted AS VARCHAR)), 'NA') AS is_seasonally_adjusted,
        COALESCE(TRIM(CAST(region AS VARCHAR)), 'NA') AS region,
        COALESCE(TRIM(CAST(city AS VARCHAR)), 'NA') AS city,
        REPLACE(COALESCE(CAST(state AS VARCHAR), 'NA'), 'U.S.', 'US') AS state,
        COALESCE(TRIM(CAST(state_code AS VARCHAR)), 'NA') AS state_code,
        COALESCE(TRIM(CAST(property_type AS VARCHAR)), 'NA') AS property_type,
        ...
        COALESCE(CAST(parent_metro_region AS VARCHAR), 'NA') AS parent_metro_region,
        COALESCE(CAST(parent_metro_region_metro_code AS VARCHAR), 'NA') AS parent_metro_region_metro_code,
...
if __name__ == '__main__':
    ...
    #
    # test_df = get_url(universal_url_string)
    # print(f"Successfully fetched data for {universal_url_string}")
    # print(f"Data head:\n{test_df.head().to_string()}")
    #
    # test_df_v2 = rdm_transformations(test_df)
    # print("RDM transformations complete.")
    #
    # test_df_v3 = sdm_tranformations(test_df_v2)
    # print("SDM transformations complete.")
    #
    # test_df_v4 = cdm_transformation(test_df_v3)
    # print("CDM transformations complete.")
    #
