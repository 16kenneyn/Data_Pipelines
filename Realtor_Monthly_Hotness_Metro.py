...
logger = DataPipelineLogger("Realtor_Monthly_Hotness_Metro")
...
def main_run():
    ...
    sql_table_name = 'REALTOR_MNTHLY_HOT_METRO'
    ...
    if sql_table_max_date: # Determining if max date is not None, if it is, the else statement will kick off the history pull.
        redfin_df = get_url(metro_history_url)
        ...
        redfin_df = get_url(metro_history_url) # Use history load url since no data was returned from sql tale.
...
if __name__ == '__main__':
    ...
    # redfin_df_v1 = get_url(metro_history_url)
    #
    # redfin_df_v2 = dataframe_etl(redfin_df_v1)
    # redfin_df_v3 = redfin_df_v2
    #
    # # Adding the date time stamp
    #
    # redfin_df_v3.to_excel('C:/Users/nickk/PycharmProjects/Data_Pipelines/2_Test/Data_Files/Realtor_Hotness/metro_hotness_current_test.xlsx', index=False)
