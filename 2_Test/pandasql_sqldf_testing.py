...
import gzip
from pandasql import sqldf # Needed if you want to run sql on pandas dataframes
...
dataframe = get_url(national_url)

query = run_sql("""
SELECT * FROM dataframe 
WHERE PERIOD_BEGIN = '2012-12-01'
...
query_2 = run_sql("""
SELECT DISTINCT
CAST(period_begin DATE) AS period_begin,
CAST(period_end DATE) AS period_end,
CAST(region_type VARCHAR(75)) AS region_type,
CAST(region_type_id INT) AS region_type_id
""")
